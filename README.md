<div align="center">

# 🏁 GEMASTIK XIX 2026 — CTF Writeups

### 🛡️ Keamanan Siber · Babak Penyisihan

*Kumpulan writeup & solver tim* **DOSCOM Zero Day Scholars**

<br>

![Event](https://img.shields.io/badge/GEMASTIK-XIX%202026-e63946?style=for-the-badge)
![Solved](https://img.shields.io/badge/SOLVED-13-2a9d8f?style=for-the-badge)
![Points](https://img.shields.io/badge/POIN-4659-f4a261?style=for-the-badge)
![Category](https://img.shields.io/badge/KATEGORI-5-457b9d?style=for-the-badge)

<br>

## 📖 &nbsp; [ Baca Writeup Lengkap → **`WRITEUP.md`** ](WRITEUP.md)

<sub>recon · analisis · exploitation · flag — 13 soal dalam satu dokumen</sub>

</div>

<br>

<div align="center">

## 👥 Tim

<table>
<tr>
<td align="center" width="210">
<a href="https://github.com/sanzxct"><img src="https://github.com/sanzxct.png" width="100" alt="sanzxcte"></a><br>
🥇 <a href="https://github.com/sanzxct"><b>sanzxcte</b></a><br>
<sub>@sanzxct</sub><br>
<img src="https://img.shields.io/badge/skor-789-E5B80B?style=flat-square">
</td>
<td align="center" width="210">
<a href="https://github.com/HugaHazimulfikri"><img src="https://github.com/HugaHazimulfikri.png" width="100" alt="nexsus404"></a><br>
🥈 <a href="https://github.com/HugaHazimulfikri"><b>nexsus404</b></a><br>
<sub>@HugaHazimulfikri</sub><br>
<img src="https://img.shields.io/badge/skor-766-9EA0A3?style=flat-square">
</td>
<td align="center" width="210">
<a href="https://github.com/x0rr-dan"><img src="https://github.com/x0rr-dan.png" width="100" alt="x0r aka teto"></a><br>
🥉 <a href="https://github.com/x0rr-dan"><b>x0r aka teto</b></a><br>
<sub>@x0rr-dan</sub><br>
<img src="https://img.shields.io/badge/skor-330-CD7F32?style=flat-square">
</td>
</tr>
</table>

</div>

---

## 📚 Daftar Challenge

> Writeup lengkap semua soal ada di **[WRITEUP.md](WRITEUP.md)**. Tiap folder soal menyimpan
> solver/exploit, file soal (bila ada), dan `img/` screenshot.

**Penanda solver** &nbsp;·&nbsp; 🔵 `nexsus404` &nbsp; 🟢 `sanzxct` &nbsp; 🟣 `x0rr-dan`

<table>
<tr><td valign="top" width="50%">

### 🔐 Crypto <sup>`4`</sup>

| Challenge | Poin | Solver |
| :-------- | :--: | :----- |
| Nonce-nse | `500` | 🔵 |
| TZKS | `499` | 🟣 |
| Ecliprime | `100` | 🟢 |
| common-encoding | `100` | 🟢 |

### 🧩 Reverse <sup>`2`</sup>

| Challenge | Poin | Solver |
| :-------- | :--: | :----- |
| hexlock | `500` | 🔵 |
| wraith | `116` | 🟢 |

### 💥 Pwn / Kernel <sup>`1`</sup>

| Challenge | Poin | Solver |
| :-------- | :--: | :----- |
| mantra | `481` | 🔵 |

</td><td valign="top" width="50%">

### 🔎 Forensics <sup>`4`</sup>

| Challenge | Poin | Solver |
| :-------- | :--: | :----- |
| Tombstone | `500` | 🔵 |
| Afterimage | `473` | 🟢 |
| Ghost in the Core | `384` | 🔵 |
| Cinder | `100` | 🔵 |

### 🌐 Web <sup>`2`</sup>

| Challenge | Poin | Solver |
| :-------- | :--: | :----- |
| BMN | `498` | 🟣 |
| Wormhole | `408` | 🟣 |

</td></tr>
</table>

---

<div align="center">

**⚡ 13 challenge solved · 4659 poin**

`crypto` · `reverse` · `forensics` · `pwn` · `web`

<sub>Made with ☕ &amp; 🚩 by <b>DOSCOM Zero Day Scholars</b></sub>

</div>
